...
function worm_track_pass(binary, is_forward_pass, img_center, arena_radius, on_circle_tolerance)::Vector{Union{Nothing, WormFeatures}}
    ...
    for current_frame in frame_iterator
        ...
        all_objects_in_frame = [WormFeatures(f.centroid, f.wormarea, f.wormperimeter, f.circularity, f.pixels_abs, f.boundary_pixels_abs) for f in find_objects_features(binary_view, MIN_AREA, MAX_AREA)]
        ...
        if !isempty(possible_worm)
            if length(possible_worm) == 1
                ...
                for (k, cand_feat) in enumerate(possible_worm)
                    ...
                    current_score = 0.8f0 * dist_score + 0.2f0 * circ_score
...
function create_track(binary)
    ...
    while frame_idx <= num_frames
        ...
        if !isnothing(worm)
            ...
            push!(perimeters, worm.wormperimeter)
            push!(circularities, worm.circularity)
            frame_idx += 1
        else
            ...
            if !isnothing(feature_before_gap) && !isnothing(feature_after_gap)
                ...
                for k in 1:num_intermediate_points
                    ...
                    push!(perimeters, feature_before_gap.wormperimeter)
                    push!(circularities, feature_before_gap.circularity)
                end
            end
        end
    end
    return positions, areas, perimeters, circularities, img_rows, img_cols
end
